...
public class test {
    ...
    private static void createMeeting(Person host) {
        ...
        System.out.println("Enter date of the meeting");
        int numberOfAttendees = input.nextInt();
        if (numberOfAttendees == 0) {
            System.out.println("Unable to create a meeting because there is no attendee");

        } else {
            ArrayList<Person> list = new ArrayList<>();
            for (int i = 0; i < numberOfAttendees; i++) {
                System.out.println("Enter the name of the attendee number " + (i + 1) + ": ");
